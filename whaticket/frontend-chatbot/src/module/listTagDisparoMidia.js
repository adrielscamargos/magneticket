...
class ListTagUser extends React.Component {
  ...
  async sendMessage(numeros, mensagem, midia, token, id, min, max) {
    if (midia === undefined) {
      alert("O campo mídia não pode estar vazio.")
    }
    else if (this.state.campMensagem === "") {
      ...
      for (const numero of numeros){
        ...
        try {
          axios.post(baseWTUrl + '/sendMedia', {
        ...
        <div>
          <div className="form-row justify-content-center">
            <div className="form-group col-md-12">
                <label htmlFor="url">Url Mídia </label>
                <input 
                ...
                required="required"
                placeholder="https://comunidadezdg.com.br/foto.jpeg"
              ></input>
            </div>
            <div className="form-group col-md-12">
          ...
          <button type="submit" className="btn btn-primary" onClick={()=>this.sendMessage(this.state.numberUser, this.state.campMensagem, this.state.campUrl, this.state.campToken, (this.state.conexao === undefined) ? this.state.conexaoInicial : this.state.conexao, this.state.campMin, this.state.campMax)}>Enviar</button>
          {'   '}
